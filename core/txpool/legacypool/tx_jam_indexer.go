...
func newTxJamIndexer(cfg TxJamConfig, pool *LegacyPool) *txJamIndexer {
	cfg = (&cfg).sanity()
...
func (indexer *txJamIndexer) updateLoop() {
	...
	for {
		...
		case <-tick.C:
			...
			pendings := indexer.pool.Pending(txpool.PendingFilter{})
