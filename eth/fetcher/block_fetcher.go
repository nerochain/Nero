...
// continuousInturnFn is a callback type for getting continuous block in turn number
type continuousInturnFn func(blockNumber *big.Int) uint64
...
type BlockFetcher struct {
	...
	continuousInturn continuousInturnFn // Gets continuous blocks in turn number
...
func NewBlockFetcher(light bool, getHeader HeaderRetrievalFn, getBlock blockRetrievalFn, verifyHeader headerVerifierFn, broadcastBlock blockBroadcasterFn, chainHeight chainHeightFn, insertHeaders headersInsertFn, insertChain chainInsertFn, dropPeer peerDropFn, continuousInturn continuousInturnFn) *BlockFetcher {
	return &BlockFetcher{
		light:            light,
		...
		dropPeer:         dropPeer,
		continuousInturn: continuousInturn,
	}
}
...
func (f *BlockFetcher) loop() {
	...
	for {
		...
		for !f.queue.Empty() {
			...
			number := op.number()
			if number > height+1 {
				if !f.hasParent(op) || number > height+f.continuousInturn(big.NewInt(int64(number)))+1 {
			...
			if len(f.completing) > 0 {
				for i := 0; i < len(task.transactions) && i < len(task.uncles); i++ {
					...
					for hash, announce := range f.completing {
						...
						matched = true
						if f.getBlock(hash) == nil {
							block := types.NewBlockWithHeader(announce.header).WithBody(types.Body{Transactions: task.transactions[i]})
							block.ReceivedAt = task.time
