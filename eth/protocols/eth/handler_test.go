...
func newTestBackendWithGenerator(blocks int, generator func(int, *core.BlockGen)) *testBackend {
	...
	gen.MustCommit(db, triedb.NewDatabase(db, triedb.HashDefaults))

	chain, _ := core.NewBlockChain(db, nil, gen, nil, ethash.NewFaker(), vm.Config{}, nil, nil)
...
func testGetNodeData(t *testing.T, protocol uint) {
	...
	for i := 0; i < len(data); i++ {
		rawdb.WriteTrieNode(reconstructDB, common.Hash{}, nil, hashes[i], data[i], rawdb.HashScheme)
	}
