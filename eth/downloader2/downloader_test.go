...
func (dlp *downloadTesterPeer) RequestHeadersByHash(origin common.Hash, amount int, skip int, reverse bool, sink chan *eth2.Response) (*eth2.Request, error) {
	// Service the header query via the live handler code
	rlpHeaders := eth2.ServiceGetBlockHeadersQuery(dlp.chain, &eth2.GetBlockHeadersRequest{
	...
	res := &eth2.Response{
		Req:  req,
		Res:  (*eth2.BlockHeadersRequest)(&headers),
		Meta: hashes,
		Time: 1,
...
// origin; associated with a particular peer in the download tester. The returned
// function can be used to retrieve batches of headers from the particular peer.
func (dlp *downloadTesterPeer) RequestHeadersByNumber(origin uint64, amount int, skip int, reverse bool, sink chan *eth2.Response) (*eth2.Request, error) {
	// Service the header query via the live handler code
	rlpHeaders := eth2.ServiceGetBlockHeadersQuery(dlp.chain, &eth2.GetBlockHeadersRequest{
		Origin: eth2.HashOrNumber{
			Number: origin,
...
func (dlp *downloadTesterPeer) RequestBodies(hashes []common.Hash, sink chan *eth2.Response) (*eth2.Request, error) {
	blobs := eth2.ServiceGetBlockBodiesQuery(dlp.chain, hashes)

	bodies := make([]*eth2.BlockBody, len(blobs))
	for i, blob := range blobs {
		bodies[i] = new(eth2.BlockBody)
	...
	res := &eth2.Response{
		Req:  req,
		Res:  (*eth2.BlockBodiesResponse)(&bodies),
...
func (dlp *downloadTesterPeer) RequestReceipts(hashes []common.Hash, sink chan *eth2.Response) (*eth2.Request, error) {
	blobs := eth2.ServiceGetReceiptsQuery(dlp.chain, hashes)
	...
	res := &eth2.Response{
		Req:  req,
		Res:  (*eth2.ReceiptsResponse)(&receipts),
		Meta: hashes,
		Time: 1,
		Done: make(chan error, 1), // Ignore the returned status
	}
...
func TestCanonicalSynchronisation68Full(t *testing.T) { testCanonSync(t, eth2.ETH68, FullSync) }
func TestCanonicalSynchronisation68Snap(t *testing.T) { testCanonSync(t, eth2.ETH68, SnapSync) }
...
func TestThrottling68Full(t *testing.T) { testThrottling(t, eth2.ETH68, FullSync) }
func TestThrottling68Snap(t *testing.T) { testThrottling(t, eth2.ETH68, SnapSync) }
...
func TestCancel68Full(t *testing.T) { testCancel(t, eth2.ETH68, FullSync) }
func TestCancel68Snap(t *testing.T) { testCancel(t, eth2.ETH68, SnapSync) }
...
func TestMultiProtoSynchronisation68Full(t *testing.T) { testMultiProtoSync(t, eth2.ETH68, FullSync) }
func TestMultiProtoSynchronisation68Snap(t *testing.T) { testMultiProtoSync(t, eth2.ETH68, SnapSync) }
...
func testMultiProtoSync(t *testing.T, protocol uint, mode SyncMode) {
	...
	tester.newPeer("peer 68", eth2.ETH68, chain.blocks[1:])
...
func TestEmptyShortCircuit68Full(t *testing.T) { testEmptyShortCircuit(t, eth2.ETH68, FullSync) }
func TestEmptyShortCircuit68Snap(t *testing.T) { testEmptyShortCircuit(t, eth2.ETH68, SnapSync) }
...
func TestBeaconSync68Full(t *testing.T) { testBeaconSync(t, eth2.ETH68, FullSync) }
func TestBeaconSync68Snap(t *testing.T) { testBeaconSync(t, eth2.ETH68, SnapSync) }
...
func TestSyncProgress68Full(t *testing.T) { testSyncProgress(t, eth2.ETH68, FullSync) }
func TestSyncProgress68Snap(t *testing.T) { testSyncProgress(t, eth2.ETH68, SnapSync) }
