...
type queue struct {
	...
	headerTaskQueue *prque.Prque[int64, any]       // Priority queue of the skeleton indexes to fetch the filling headers for
	...
	blockTaskQueue *prque.Prque[int64, any]      // Priority queue of the headers to fetch the blocks (bodies) for
	...
	receiptTaskQueue *prque.Prque[int64, any]      // Priority queue of the headers to fetch the receipts for
...
func newQueue(blockCacheLimit int, thresholdInitialSize int) *queue {
	...
	q := &queue{
		...
		blockTaskQueue:   prque.New[int64, any](nil),
		receiptTaskQueue: prque.New[int64, any](nil),
...
func (q *queue) ScheduleSkeleton(from uint64, skeleton []*types.Header) {
	...
	q.headerTaskQueue = prque.New[int64, any](nil)
...
func (q *queue) reserveHeaders(p *peerConnection, count int, taskPool map[common.Hash]*types.Header, taskQueue *prque.Prque[int64, any],
...
func (q *queue) cancel(request *fetchRequest, taskQueue *prque.Prque[int64, any], pendPool map[string]*fetchRequest) {
	if request.From > 0 {
...
// reason the lock is not obtained in here is because the parameters already need
// to access the queue, so they already need a lock anyway.
func (q *queue) expire(timeout time.Duration, pendPool map[string]*fetchRequest, taskQueue *prque.Prque[int64, any], timeoutMeter metrics.Meter) map[string]int {
...
func (q *queue) deliver(id string, taskPool map[common.Hash]*types.Header,
	taskQueue *prque.Prque[int64, any], pendPool map[string]*fetchRequest, reqTimer metrics.Timer,
