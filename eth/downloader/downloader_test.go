...
func (dl *downloadTester) SnapSyncCommitHead(hash common.Hash) error {
	// For now only check that the state trie is correct
	if block := dl.GetBlockByHash(hash); block != nil {
		_, err := trie.NewStateTrie(trie.StateTrieID(block.Root()), triedb.NewDatabase(dl.stateDb, nil))
		return err
	}
...
// with trie nodes.
func (dl *downloadTester) TrieDB() *triedb.Database {
	return triedb.NewDatabase(dl.stateDb, nil)
}
...
func checkSyncMode(t *testing.T, mode SyncMode) {
	if mode == FastSync {
		t.Skip("FastSync is removed")
	}
}
